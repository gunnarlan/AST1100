...
class Launch_simulation:
	...
	def start_orbital_injection(self, t, position, velocity):
		...
		print "Injection velocity:", np.linalg.norm(injection_velocity)
	...
	#	plt.text(position_satellite[0,min_index], position_satellite[1,min_index], str(min_index))
		...
		#plt.plot(self.positionFunction(time_array[min_index])[0,self.target_planet], self.positionFunction(time_array[min_index])[1,self.target_planet],  marker='*')
		#plt.text(self.positionFunction(time_array[min_index])[0,self.target_planet], self.positionFunction(time_array[min_index])[1,self.target_planet], str(min_index))
...
launch.launch_satellite(N_per_year=2000000, time=0.1, t_0=time_start, initial_position=position_start, initial_v=velocity_start, delta_v_for_boost=delta_v, delta_t_for_boost=0.005435) #0.005435
