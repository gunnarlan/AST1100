...
class Find_Velocity():
    ...
    def __init__(self):
        ...
        self.m_s_to_AU_year=(365.24*24*60*60)/(1.49597871*1e11)
    ...
    def compute_velocity_of_sat(self, d_lamb_wrt_star_1, d_lamb_wrt_star_2, d_lamb_wrt_sat_1, d_lamb_wrt_sat_2, phi_1, phi_2):
        if abs(np.sin(phi_2-phi_1))<1e-18:
        ...
        print "Velocity of star 1 [AU/year]:",  v_refstar_1*self.m_s_to_AU_year
        print "Velocity of star 2 [AU/year]:",  v_refstar_2*self.m_s_to_AU_year
	...
	print "Velocity of satellite in the direction of the stars [AU/year]", v_sat*self.m_s_to_AU_year
        prefac=1.0/float(np.sin(phi_2-phi_1))
        A_inv=np.array(([np.sin(phi_2), -np.sin(phi_1)], [-np.cos(phi_2), np.cos(phi_1)]))
        v=self.m_s_to_AU_year*prefac*(A_inv.dot(v_sat))
        print "Velocity of satelite in star reference system [AU/year]", v

if __name__ == "__main__":

	"""
	Test if there is no shift
	"""
	print "No shift:"
	...
	"""
	Test if there is the shift is the same
	"""
	print "Same shift as planet:"
	lambda_1_wrt_star=-0.020159867169*1e-9
	lambda_2_wrt_star=-0.014180672906*1e-9
	lambda_1_wrt_sat=-0.020159867169*1e-9
	lambda_2_wrt_sat=-0.014180672906*1e-9
	phi_1=np.deg2rad(171.006135)
	phi_2=np.deg2rad(87.754469)
	velocity=Find_Velocity()
	velocity.compute_velocity_of_sat(lambda_1_wrt_star, lambda_2_wrt_star, lambda_1_wrt_sat, lambda_2_wrt_sat, phi_1, phi_2)
