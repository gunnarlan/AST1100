...
class Planet_Atmosphere():
	def __init__(self, seed, mu, gamma):
		...
		self.A=6
		print "g:", self.G*self.M_in_kg/float(self.radius_planet_in_m**2)
	...
	def compute_critical_r(self, eps):
		"""
		Computes the point where the atmosphere is less than a specified number eps
		"""
		treshold=1e-20
		if eps < treshold:
			print "Please choose an limit larger than ", treshold
			sys.exit()
	...
	def find_root_closest_r(self):
		"""
		Unused method, used to determine where the fraction of forces is larger than 1000
		"""
		r_isothermal=self.compute_r_T_2()
	...
	def compute_atmosphere_numeric(self):
		...
		rho_diff=100*(rho_tot_analytic-rho)/rho_tot_analytic

		print "Mean diff:",np.mean(rho_diff)
		print "Max diff:", max(rho_diff)
		...
		plt.title('Atmosphere of Hiffre')
		plt.xlabel('Distance from center of Hiffre [planet radii]')
