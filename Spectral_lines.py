...
class Find_Atmosphere():
	...
	def __init__(self, seed, lambda_0, mass_of_molecule):
		...
		if self.lambda_0 < 600+boundary_lambda:
			...
			sys.exit(1)
		#self.lower_index_of_interest=int((lambda_0-600+boundary_lambda)/(0.00024))
		#self.upper_index_of_interest=int((lambda_0-599-boundary_lambda)/(0.00024))
	...
	def Chi_square(self):
		...
		print "Chosen lambda:", self.lambda_0
		print "Lowest chi-square:",  chi_square[index_F, index_sigma, index_lambda]
		print "F_min:", F_min[index_F]
		print "Sigma:", sigma[index_sigma]
		print "Lambda center:", lambda_center[index_lambda]
		...
		#guess=1+(F_min[index_F]-1)*np.exp(-((interesting_lambdas-lambda_center[index_lambda])**2)/(2*(sigma[index_sigma])**2))
