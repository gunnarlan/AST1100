...
class Orbital_Simulations():
	def __init__(self, seed, target_planet=6, atmosphere_tolerance=1e-10):
		self.seed=seed
	...
	def compute_first_boost_hohmann(self, position):
		...
		deltav=np.sqrt(mu/(float(r)))*(np.sqrt(2.0*self.r_critical/(self.r_critical+r))-1)
	...
	def simulate_satellite_orbit(self, N_per_second, seconds, initial_position, initial_velocity,t_0=0):
		...
		for i in xrange(0, int(N_per_second*seconds)-1):
			...
			if i > 10000 and already_boosted == False:
				velocity_satellite+=(deltav_1)*(initial_velocity)/float(np.linalg.norm(initial_velocity))
				print "First boost"
			...
			if distance_to_target_planet[-1] < self.r_critical+1e6 and already_boosted==False:
				velocity_satellite = velocity_satellite-0.05*velocity_satellite
				already_boosted=True
				print "Boosting!"

		print velocity_satellite
		print position_satellite[:, -1]
		...
		#plt.savefig('Initial_orbit_satellite.pdf')
		plt.show()
...
initial_pos_in_m=initial_pos_in_au*AU_in_m
initial_vel_in_m_s=initial_vel_in_au_year*AU_in_m/(float(years_in_seconds))

seconds_in_year=300*30*60*60

launch=Orbital_Simulations(6392)
launch.simulate_satellite_orbit(N_per_second=0.1, seconds=0.04*seconds_in_year, initial_position=initial_pos_in_m, initial_velocity=initial_vel_in_m_s, t_0=time_0_in_seconds)
