...
class Launch_simulation:
	...
	def write__initial_text_file(self, t0, x_0, y_0, vx0, vy0, fuel=23000, dp_dt=1.71864513374e-9, no_boxes=8.93689723171e+12, particles_per_sec=6.3528e+13):
		outfile=open("Satellite_instructions.txt", 'w')
		outfile.write("launch "+str(t0)+" "+str(x_0)+ " "+str(y_0)+ " "+str(vx0)+ " "+str(vy0)+ " "+str(fuel)+ " "+ str(dp_dt)+ " "+str(no_boxes)+" "+str(particles_per_sec))
		outfile.close()
	...
	def launch_satellite(self, N_per_year, time, max_close,  t_0=0, reduced_dt=1000, reduced_dt_target=100, time_to_close_planet=1.35):
		...
		corrected_v=1.314
		print "Delta v for first boost is:", deltav-corrected_v #deltav - 0.7
		velocity_satellite+=(vel_home/(float((np.linalg.norm(vel_home)))))*(deltav-corrected_v) #deltav-0.7
		vx0=((vel_home/(float((np.linalg.norm(vel_home)))))*(deltav-corrected_v))[0]
		vy0=((vel_home/(float((np.linalg.norm(vel_home)))))*(deltav-corrected_v))[1]
		...
		self.write__initial_text_file(t_0, position_satellite[0, 0], position_satellite[1,0], vx0, vy0)
...
launch.launch_satellite(20000, 4.0, max_close=1.0/100, t_0=3.5909) #1.4 #3.7451
